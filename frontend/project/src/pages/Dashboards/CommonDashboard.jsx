...
const getVegImage = (veg) => {
  if (!veg) return defaultImg;
  if (VEG_IMAGE_MAP[veg]) return VEG_IMAGE_MAP[veg];

  const v = veg.toLowerCase();
  if (v.includes("onion")) return onionImg;
  if (v.includes("potato")) return potatoLocalImg;
  if (v.includes("tomato")) return tomatoImg;
  if (v.includes("chilli") || v.includes("chili")) return chilliImg;
  if (v.includes("carrot")) return carrotImg;
  if (v.includes("lime")) return limeImg;
  if (v.includes("snake")) return snakeGourdImg;
...
export default function CommonDashboard() {
  ...
  const chartData = useMemo(() => {
    const filtered = stocks.filter((s) => {
      if (s.vegetable !== selectedVeg) return false;
      if (selectedMarket !== "All" && s.market !== selectedMarket)
        return false;
      return true;
    });
